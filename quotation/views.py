...
def get_form_data(request):
    ...
    if request.method == "POST":
        ...
        for room in post_data['roomData']:
            ...
            ###
            startEndDateFilter = lambda hotelRate : (hotelRate.date_applicable_from <= start_time.date() <= end_time.date() <= hotelRate.date_applicable_to)
            ids = [hotelRate.id for hotelRate in HotelRateInstance if startEndDateFilter(hotelRate)]
            # HotelRateInstance = HotelRate.objects.filter(id__in=ids).order_by('id','-date_created').distinct()
            HotelRateInstance = HotelRate.objects.filter(id__in=ids)
            ###
            print(HotelRateInstance)
            print(HotelRateInstance.values_list('id'))
